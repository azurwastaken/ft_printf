...
char	*parse_length(char *format, t_flag *flag)
{
		...
		else if (*format == 'l' && (flag->length == NULL || is_charset(flag->length[0],"h")))
		{
			flag->length = format[1] == 'l' ? ft_strdup("ll") : ft_strdup("l");
			format += ft_strcmp(flag->length, "ll") == 0 ? 2 : 1;
			flag->is_length = 1;
		}
		else if (*format == 'j' && (flag->length == NULL || is_charset(flag->length[0],"hlz")))
		{
			flag->length = ft_strdup("j");
			format += (flag->is_length = 1);
		}
		else if (*format == 'z' && (flag->length == NULL || is_charset(flag->length[0],"hl")))
		{
